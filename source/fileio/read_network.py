...
class ReadNetworkHexagonal(ReadNetwork):
    ...
    def read(self, flownetwork):
        ...
        if nr_of_hexagon_x % 2 == 0 or nr_of_hexagon_y % 2 == 0:
            raise SystemExit("Error: Choose odd number of hexagon.")
        ...
        dy = float(vessel_length * math.sin(alpha))
        dx_s = float(vessel_length + 2*vessel_length*math.cos(alpha))
        ...
        bc_edges = [np.where(np.any(edge_list == valor, axis=1))[0][0] for valor in flownetwork.boundary_vs]
        flownetwork.bc_edges = np.sort(np.array(bc_edges))
...
class ReadNetworkCsv(ReadNetwork):
    ...
    def read(self, flownetwork):
        ...
        df_boundary_data.sort_values(self._PARAMETERS["csv_boundary_vs"], inplace=True)
...
class ReadNetworkIgraph(ReadNetwork):
    def read(self, flownetwork):
        ...
        bc_edges = [np.where(np.any(flownetwork.edge_list == valor, axis=1))[0][0] for valor in flownetwork.boundary_vs]
        flownetwork.bc_edges = np.sort(np.array(bc_edges))
...
class ReadTortuousNetwork(ReadNetwork):
    """
    Class for reading a vascular tortuous network from a csv file. It reads two pkl files: edges_pkl and vertices_pkl.
    edges_data and vertices_data are dictionaries.
    """
    def read(self, flownetwork):
        # Open the .pkl file in binary read mode ('rb') with specific protocol
        with open(self._PARAMETERS["edges_pkl"], 'rb') as file:
            # Load data from the .pkl file using the appropriate protocol
            edges_data = pickle.load(file, encoding='latin1')

        with open(self._PARAMETERS["vertices_pkl"], 'rb') as file:
            # Load data from the .pkl file using the appropriate protocol
            vertices_data = pickle.load(file, encoding='latin1')

        flownetwork.diameter = np.array(edges_data['diameter']) * 1e-6
        # To compare with the results obtained during stroke changing diameter in the input network
        # flownetwork.diameter[205] *= 0.01
        flownetwork.length = np.array(edges_data['length']) * 1e-6

        edge_list = np.array(edges_data['connectivity'])
        flownetwork.edge_list = edge_list
        xyz = np.array(vertices_data['coords'])
        flownetwork.xyz = xyz

        flownetwork.diameters_points = edges_data['diameters']
        flownetwork.inner_edges_lengths = edges_data['lengths2']
        flownetwork.tortuous_points = edges_data['points']

        # Network attributes
        flownetwork.nr_of_vs = np.size(xyz, 0)
        flownetwork.nr_of_es = np.size(edge_list, 0)

        # Select elements that appear exactly once
        import pandas as pd
        boundary_data_path = self._PARAMETERS["boundary_data"]
        boundary_vs = pd.read_csv(boundary_data_path)['nodeID'].to_numpy()
        bc_edges_0 = [np.where(np.any(edge_list == valor, axis=1))[0][0] for valor in boundary_vs]
        bc_edges = np.sort(np.array(bc_edges_0))

        boundary_data_df = pd.read_csv(boundary_data_path)
        selected_edge_pairs = flownetwork.edge_list[bc_edges]
        boundary_vs_sorted = np.array([pair[0] if pair[0] in boundary_vs else pair[1]
                                for pair in selected_edge_pairs])
        # Set 'vertex_param_vid' as the index in df_parameter_space
        boundary_data_df = boundary_data_df.set_index('nodeID')
        ...
        sorted_df = boundary_data_df.reindex(boundary_vs_sorted)
        ...
        flownetwork.boundary_vs = sorted_df['nodeID'].to_numpy()
        flownetwork.boundary_type = sorted_df['boundaryType'].to_numpy()
        flownetwork.boundary_val = sorted_df['p'].to_numpy()
        bc_edges = [np.where(np.any(edge_list == valor, axis=1))[0][0] for valor in flownetwork.boundary_vs]
        flownetwork.bc_edges = np.sort(np.array(bc_edges))
